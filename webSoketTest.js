...
var server = app.listen(()=>{
    console.log('Listening at port number') 
})

//return socket.io server.
var io = socketio(server) // 이 과정을 통해 우리의 express 서버를 socket io 서버로 업그레이드를 시켜줍니다.
...
io.on('connection',function (socket){
    ...
    socket.on('say',function(data){
        ...
        socket.emit('myMsg',data)//(소켓 사용자)자기 자신한테만
        io.emit('newMsg',"수신됨1")//("모두한테")
        socket.broadcast.emit('newMsg',"수신됨") // socket.broadcast.emit은 현재 소켓이외의 서버에 연결된 모든 소켓에 보내는 것.
    })
    ...
    socket.on('logout',function(){
        ...
        socket.broadcast.emit('updateChat', "adsf")
